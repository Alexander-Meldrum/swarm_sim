...
# ============================================================
OBS_DIM = 4 + 2 * 9  # Observation dimension (self state + 2 * neighbors obs etc.)
...
def main():
    ...
    # MAIN TRAINING LOOP
    # ============================================================
    # episode counter incremented 1 at simulator reset. First/Last episode: 1 -> EPISODE_COUNT
    for episode in range(1, EPISODE_COUNT + 1):
        ...
        while not done:
            ...
            if step_count % ROLLOUT_STEPS == 0 or done:
                ...
                for _ in range(PPO_EPOCHS):
                    for start in range(0, batch_size, MINIBATCH_SIZE):
                        ...
                        value_pred_clipped = value_batch[mb_idx] + (
                            value_pred - value_batch[mb_idx]
                        ).clamp(
                            -0.5, 0.5
                        )  # PPO2-style value clipping

                        value_loss_unclipped = (
                            value_pred - returns[mb_idx].detach()
                        ) ** 2
                        value_loss_clipped = (
                            value_pred_clipped - returns[mb_idx].detach()
                        ) ** 2
                ...
                # ============================================================
                if DEBUG_PRINTS:
                    print(f"\nPPO batch optimization done after {step_count} steps")
                    ...
                    print(
                        "action mean:",
                        action.mean(dim=0).detach().cpu().numpy(),
                        "action std :",
                        action.std(dim=0).detach().cpu().numpy(),
                    )
                ...
                if EXPORT_POLICY and episode % 100 == 0:
                    torch.save(
                        {
                            "episode": episode,
